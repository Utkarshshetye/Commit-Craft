...
void handle_pull(int client_fd, const char *branch) {
    ...
    size_t bytes_read;
    while ((bytes_read = fread(content, 1, sizeof(content), file)) > 0) {
...
void handle_push(int client_fd, const char *branch, const char *file_name, size_t file_size) {
    char file_path[256];
    ...
    // Write the received file data to the temporary file
...
void handle_history(int client_fd, const char *branch) {
    ...
    char response[BUFFER_SIZE] = "Commit history:\n";
    while (strlen(hash) > 0) {
...
void handle_client(int client_fd) {
	...
	if (strcmp(command,"PUSH") == 0) {
        handle_push(client_fd, branch, file_name, file_size);
	}
